...
const ProjectDashboardView: React.FC<ProjectDashboardViewProps> = ({ onSelect }) => {
  ...
  useEffect(() => {
    ...
    void callCore<{ projects: Project[] }>('/api/v1/projects', { method: 'GET' })
      .then((response) => {
        if (!cancelled) {
        ...
        if (!cancelled) {
          ...
          setError(
            /bearer token|unauthorized|invalid bearer/i.test(message)
              ? 'Core kraver en giltig admin- eller BankID-session for att ladda riktiga projekt.'
              : message,
          );
        }
      })
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 space-y-6 fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500">Översikt av pågående miljöprövningar och datatäckning.</p>
      </div>
