...
import type { Project, SearchResult } from './mvpDemoModel';
import { Badge, Card } from './mvpDemoShared';

type MvpDocumentSearchViewProps = {
  project: Project;
};

const MvpDocumentSearchView: React.FC<MvpDocumentSearchViewProps> = ({ project }) => {
  ...
  const performSearch = async () => {
    ...
    setLoading(true);
    try {
      const response = await callMvp<{ results?: SearchResult[] }>(`/api/v1/projects/${project.id}/search`, {
        method: 'GET',
    ...
    <div className="animate-in slide-in-from-left-4 space-y-6 fade-in duration-400">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mb-6 text-sm text-slate-500">
          Hybrid-sökning (semantisk + nyckelord) mot RAG-indexerade dokument för{' '}
        ...
        {results.map((result) => (
          <Card key={result.id} className="border-l-4 border-l-indigo-500 p-5">
            <div className="mb-2 flex items-start justify-between">
              ...
              <Badge label={`${Math.round(result.score * 100)}% Match`} color="bg-indigo-50 text-indigo-700" />
            </div>
        ...
        {!loading && results.length === 0 && query ? (
          <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 py-20 text-center">
            <p className="font-medium text-slate-400">
              Inga träffar på "{query}". Prova en bredare sökning.
            </p>
          </div>
        ) : null}
      </div>
...
export default MvpDocumentSearchView;
